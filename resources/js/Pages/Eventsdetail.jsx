...
import { Clock, MapPin, ArrowLeft, CalendarDays } from "lucide-react";
import Wrapper from "@/FrontWrapper/Wrapper";
...
export default function EventDetails({ eventId }) {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        setLoading(true);
        const id = eventId || window.location.pathname.split("/").pop();
  ...
  return (
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="relative w-full h-[45vh] sm:h-[55vh] min-h-[320px] overflow-hidden">
        <img
          src={event.image_path ? `/storage/${event.image_path}` : "/placeholder-image.jpg"}
          alt={event.title}
          className="absolute inset-0 w-full h-full object-cover"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-black/10" />
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-10 lg:px-0 py-12 sm:py-16 grid grid-cols-1 lg:grid-cols-3 gap-10 lg:gap-14">
        <div className="lg:col-span-2 flex flex-col gap-10">
          <div>
            ...
            <div className="text-gray-600 text-sm sm:text-base leading-relaxed prose max-w-none">
              {event.description ? parse(event.description) : (event.short_description || "")}
            </div>
          </div>
        </div>
